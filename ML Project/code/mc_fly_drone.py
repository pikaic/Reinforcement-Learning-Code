...
def drone_motion():
    #############################################
    ##### PATH PLANNER & DRONE MOTION BY MC######
    #############################################

    P = env.env.P

    # due to problems with FrozenLake environemnt we have to introduce changes in rewards and probability transitions
    # changes introduce only for this course only
    ...
    ######################################
    """
    HERE YOU CAN CHANGE THE GOAL FOR THE DRONE.
    DEFAULT IS AAA.
    """
    ...
    def drone_flying_plan(policy):
        drone_state = [0,0]
        motion_plan = []
        motion_plan.append(drone_state) #adding start state
    ...
    #print("drone motion : ", drone_motion(flying_plan, policy))
...
if __name__ == '__main__':
    motion = drone_motion()
    print(motion)
    #motion = [1,0,-1,0,-1,0] #goal 2 [1, 0, -1] #goal 9  [1, 0, -1, 0, 1, -1] #goal 15                 # [1,0,-1,0,-1,0] #goal 2
